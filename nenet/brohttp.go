...
func (t *Tabs) Send() ([]string, string, error) {
	...
	defer cancel()
	ccctx, cccancel := context.WithTimeout(ctx, time.Second*120)
	defer cccancel()
	subtabctx, subtabcancel := context.WithCancel(ccctx)
	defer subtabcancel()
	zzctx, zzcancel := context.WithTimeout(subtabctx, time.Second*15)
	defer zzcancel()
